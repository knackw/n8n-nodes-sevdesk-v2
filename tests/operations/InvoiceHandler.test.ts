...
describe('InvoiceHandler', () => {
  ...
  describe('buildCustomRequest', () => {
    ...
    it('should build sendByEmail request correctly', () => {
      ...
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('invoiceId', 0);
    ...
    it('should build bookAmount request correctly', () => {
      ...
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('invoiceId', 0);
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('amount', 0);
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('date', 0);
    });
    ...
    it('should call parent buildCustomRequest for unknown operations', () => {
      const spy = jest.spyOn(Object.getPrototypeOf(Object.getPrototypeOf(invoiceHandler)), 'buildCustomRequest');
  ...
  describe('CRUD Operations Integration', () => {
    ...
    it('should execute create operation with date transformation', async () => {
      ...
      mockExecuteFunctions.getNodeParameter
        .mockReturnValueOnce(invoiceData) // for data parameter
    ...
    it('should execute update operation', async () => {
      ...
      mockExecuteFunctions.getNodeParameter
        .mockReturnValueOnce(invoiceId) // for invoiceId
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid operation', async () => {
      await expect(invoiceHandler.execute('invalidOperation', 0)).rejects.toThrow();
    });
    ...
    it('should handle missing required parameters for custom operations', async () => {
      ...
      await expect(invoiceHandler.execute('sendByEmail', 0)).rejects.toThrow();
    });
  ...
  describe('Validation', () => {
    it('should validate supported operations', () => {
      const supportedOperations = [
        ...
        'sendByEmail', 'bookAmount', 'cancelInvoice', 'getPositions'
      ];

      supportedOperations.forEach(operation => {
        expect(() => (invoiceHandler as any).validateOperation(operation)).not.toThrow();
      });
    });

    it('should reject unsupported operations', () => {
      expect(() => (invoiceHandler as any).validateOperation('unsupportedOperation')).toThrow();
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle network timeout', async () => {
      ...
      await expect(invoiceHandler.execute('get', 0)).rejects.toThrow('ETIMEDOUT');
    });
