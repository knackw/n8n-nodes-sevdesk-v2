...
describe('VoucherHandler', () => {
  ...
  describe('buildCustomRequest', () => {
    ...
    it('should build uploadFile request correctly', () => {
      ...
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('voucherId', 0);
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('filename', 0);
    });
    ...
    it('should call parent buildCustomRequest for unknown operations', () => {
      const spy = jest.spyOn(Object.getPrototypeOf(Object.getPrototypeOf(voucherHandler)), 'buildCustomRequest');
  ...
  describe('transformCreateData', () => {
    ...
    it('should handle invalid date formats gracefully', () => {
      ...
      expect(() => {
        (voucherHandler as any).transformCreateData(testData);
      }).toThrow();
    });
  });
  ...
  describe('CRUD Operations Integration', () => {
    beforeEach(() => {
      ...
      (mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockResolvedValue({
        objects: [{ id: '12345', objectName: 'Voucher', voucherNumber: 'VOU-001' }],
        total: 1,
      });
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid date formats in transformations', () => {
      ...
      expect(() => {
        (voucherHandler as any).transformCreateData(invalidData);
      }).toThrow();
    });
  });
  ...
  describe('Validation', () => {
    it('should validate supported operations', () => {
      const supportedOperations = [
        ...
        'uploadFile', 'bookVoucher', 'getPositions', 'getAccountingTypes'
      ];

      supportedOperations.forEach(operation => {
        expect(() => (voucherHandler as any).validateOperation(operation)).not.toThrow();
      });
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle complex voucher data with positions', () => {
      ...
      const result = (voucherHandler as any).transformCreateData(complexVoucherData);
