...
describe('ContactHandler', () => {
  ...
  describe('buildCustomRequest', () => {
    ...
    it('should build checkCustomerNumberAvailability request correctly', () => {
      ...
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('customerNumber', 0);
    });
    ...
    it('should build findByCustomFieldValue request correctly', () => {
      ...
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('customFieldSetting', 0);
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('value', 0);
    });

    it('should call parent buildCustomRequest for unknown operations', () => {
      const spy = jest.spyOn(Object.getPrototypeOf(Object.getPrototypeOf(contactHandler)), 'buildCustomRequest');
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid operation', async () => {
      await expect(contactHandler.execute('invalidOperation', 0)).rejects.toThrow();
    });
  ...
  describe('Validation', () => {
    it('should validate supported operations', () => {
      const supportedOperations = [
        ...
        'checkCustomerNumberAvailability', 'findByCustomFieldValue'
      ];

      supportedOperations.forEach(operation => {
        expect(() => (contactHandler as any).validateOperation(operation)).not.toThrow();
      });
    });

    it('should reject unsupported operations', () => {
      expect(() => (contactHandler as any).validateOperation('unsupportedOperation')).toThrow();
    });
  });
  ...
  describe('Data Transformation', () => {
    it('should handle complex contact data transformation', () => {
      ...
      const result = (contactHandler as any).transformCreateData(complexContactData);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle network timeout', async () => {
      ...
      await expect(contactHandler.execute('get', 0)).rejects.toThrow('ETIMEDOUT');
    });
  });
});
