...
describe('OrderHandler', () => {
  ...
  describe('buildCustomRequest', () => {
    ...
    it('should build sendByEmail request correctly', () => {
      ...
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('orderId', 0);
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('sendType', 0, 'VPR');
    });
    ...
    it('should build createInvoiceFromOrder request correctly', () => {
      ...
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('orderId', 0);
      expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('invoiceDate', 0);
    });
    ...
    it('should call parent buildCustomRequest for unknown operations', () => {
      const spy = jest.spyOn(Object.getPrototypeOf(Object.getPrototypeOf(orderHandler)), 'buildCustomRequest');
      ...
      expect(spy).toHaveBeenCalledWith(baseOptions, baseURL, 'unknownOperation', 0);
  ...
  describe('CRUD Operations Integration', () => {
    ...
    it('should execute create operation with date transformation', async () => {
      ...
      mockExecuteFunctions.getNodeParameter
        .mockReturnValueOnce(orderData) // for data parameter
    ...
    it('should execute update operation', async () => {
      ...
      mockExecuteFunctions.getNodeParameter
        .mockReturnValueOnce(orderId) // for orderId
  ...
  describe('Custom Operations Integration', () => {
    ...
    it('should execute createInvoiceFromOrder operation', async () => {
      ...
      expect(mockExecuteFunctions.helpers.httpRequest).toHaveBeenCalledWith(
        expect.objectContaining({
          method: 'POST',
          url: expect.stringContaining(`/Order/${orderId}/createInvoiceFromOrder`),
          body: { invoiceDate },
        })
      );
      expect(result).toBeDefined();
    ...
    it('should execute createPackingListFromOrder operation', async () => {
      ...
      const result = await orderHandler.execute('createPackingListFromOrder', 0);

      expect(mockExecuteFunctions.helpers.httpRequest).toHaveBeenCalledWith(
        expect.objectContaining({
          method: 'POST',
          url: expect.stringContaining(`/Order/${orderId}/createPackingListFromOrder`),
        })
      );
      expect(result).toBeDefined();
    });
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      ...
      (mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockRejectedValue(apiError);
    ...
    it('should handle invalid operation', async () => {
      await expect(orderHandler.execute('invalidOperation', 0)).rejects.toThrow();
    });
    ...
    it('should handle missing required parameters for custom operations', async () => {
      ...
      await expect(orderHandler.execute('sendByEmail', 0)).rejects.toThrow();
    });
  ...
  describe('Validation', () => {
    it('should validate supported operations', () => {
      const supportedOperations = [
        ...
        'sendByEmail', 'getPositions', 'createInvoiceFromOrder', 'createPackingListFromOrder'
      ];

      supportedOperations.forEach(operation => {
        expect(() => (orderHandler as any).validateOperation(operation)).not.toThrow();
      });
    });

    it('should reject unsupported operations', () => {
      expect(() => (orderHandler as any).validateOperation('unsupportedOperation')).toThrow();
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle malformed API response', async () => {
      (mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockResolvedValue(null);
      mockExecuteFunctions.getNodeParameter.mockReturnValue('12345');
    ...
    it('should handle network timeout', async () => {
      ...
      await expect(orderHandler.execute('get', 0)).rejects.toThrow('ETIMEDOUT');
    });
    ...
    it('should handle complex order data with positions', () => {
      ...
      const result = (orderHandler as any).transformCreateData(complexOrderData);
