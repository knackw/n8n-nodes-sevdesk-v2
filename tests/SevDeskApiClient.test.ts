...
describe('SevDeskApiClient - Functional Tests', () => {
	...
	describe('Initialization', () => {
		it('should initialize with credentials successfully', async () => {
			...
			expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('sevDeskApi');
		});

		it('should handle missing credentials gracefully', async () => {
			mockExecuteFunctions.getCredentials.mockRejectedValue(new Error('Credentials not found'));

			await expect(apiClient.initialize()).rejects.toThrow('Credentials not found');
		});
	});
	...
	describe('Authentication', () => {
		...
		it('should use correct API version in base URL', async () => {
			...
			expect(mockExecuteFunctions.helpers.httpRequest).toHaveBeenCalledWith(
				expect.objectContaining({
					url: expect.stringContaining('api/v3'),
				})
			);
		});
	});
	...
	describe('Error Handling', () => {
		...
		it('should handle 401 Unauthorized errors', async () => {
			...
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockRejectedValue(unauthorizedError);
		...
		it('should handle 404 Not Found errors', async () => {
			...
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockRejectedValue(notFoundError);
		...
		it('should handle 429 Rate Limit errors', async () => {
			...
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockRejectedValue(rateLimitError);
	...
	describe('Response Processing', () => {
		...
		it('should handle empty responses', async () => {
			...
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockResolvedValue(emptyResponse);
		...
		it('should handle malformed responses gracefully', async () => {
			...
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockResolvedValue(malformedResponse);
	...
	describe('Logging and Debugging', () => {
		...
		it('should log errors for debugging', async () => {
			...
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockRejectedValue(error);
	...
	describe('Integration Scenarios', () => {
		...
		it('should handle complete CRUD workflow', async () => {
			...
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockResolvedValueOnce(createResponse);

			const createResult = await apiClient.post('/Contact', { name: 'Test Company' });
			...
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockResolvedValueOnce(readResponse);
			...
			const updateResponse = {
				objects: [TestDataFactory.createMockContact({ name: 'Updated Company' })],
				total: 1,
			};
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockResolvedValueOnce(updateResponse);

			const updateResult = await apiClient.put('/Contact/12345', { name: 'Updated Company' });
			expect(updateResult.data).toEqual(updateResponse);

			// Delete
			const deleteResponse = { objects: [], total: 0 };
			(mockExecuteFunctions.helpers.httpRequest as jest.MockedFunction<any>).mockResolvedValueOnce(deleteResponse);
